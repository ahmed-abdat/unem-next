...
import SharePoste from "../news/poste/SharePoste";
...
function Poste({ poste , id } : { poste: Accademics | null , id: string}) {
  ...
  const url = `https://unem2000.com${path}`;
  console.log(url);
  ...
  return (
        <>
          ...
          <section className="pt-6 bg-white min-h-[85dvh] sm:py-[20px] sm:px-4 ">
            ...
            {
              isClient ? <Thumbnail imageSrc={poste?.thumbnail ?? poste?.images[0]?.url ?? ''}  /> : <Skeleton className="w-full h-[300px] md:max-w-[70dvw]" />
            }
            ...
            <div className="px-3 mt-4 grid grid-cols-mobile gap-[0.3rem] sm:grid-cols-tablet sm:gap-[0.4rem] lg:grid-cols-desktop w-full h-full">
              {poste?.images.map((image: { url: string; alt: string }) => (
                <div
